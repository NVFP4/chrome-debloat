...
pub fn render(frame: &mut Frame<'_>, area: Rect, app: &crate::app::App) {
    if input_footer(app).is_some() {
    ...
    render_space_between(frame, row_area, left, right);
}

fn footer_lines(width: u16, app: &crate::app::App) -> (Line<'static>, Line<'static>) {
    if let Some((full_label, compact_label, shortcuts)) = input_footer(app) {
        return (
            input_footer_line(usize::from(width), full_label, compact_label, shortcuts),
            Line::default(),
        );
    }
...
fn input_footer(app: &crate::app::App) -> Option<InputFooter> {
